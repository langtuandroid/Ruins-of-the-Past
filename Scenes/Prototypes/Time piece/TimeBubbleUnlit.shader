Shader "Unlit/Time Bubble Unlit"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float4 color;
                float2 screen_space_uv = i.screenUv.xy / i.screenUv.z;

                // Flip y if OpenGL UVs
                #ifdef UNITY_UV_STARTS_AT_TOP
                screen_space_uv.y = 1 - screen_space_uv.y;
                #endif

                float2 uv = screen_space_uv;
                uv.x += _Time.x * _ScrollSpeedX;
                uv.y += _Time.x * _ScrollSpeedY;

                // Calculate normal from texture
                half3 norm = UnpackNormal(tex2D(_NoiseTex, uv));
                ...
                float4 tex = tex2D(_MainTex, screen_space_uv + _Distortion * refraction_vector);
